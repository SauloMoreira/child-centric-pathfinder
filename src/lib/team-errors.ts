...
export const teamErrorMessages: Record<string, string> = {
  ...
  NO_ACTIVE_ORGANIZATION: "Você precisa ter um órgão de execução ativo para gerenciar a equipe.",
  TEAM_INVITATION_ALREADY_PENDING: "Já existe um convite pendente para este e-mail neste órgão.",
  USER_ALREADY_MEMBER_OF_ORGANIZATION: "Este usuário já faz parte da equipe deste órgão.",
  ...
  MEMBER_INCOMPATIBLE_ROLE: "Este usuário possui papel incompatível para vínculo como membro.",
  ...
  DEFENDER_HAS_NO_ORG: "Você precisa possuir um órgão de execução ativo antes de vincular membros.",
  ...
  CONCURRENT_CHANGE: "Os dados foram alterados por outro processo. Recarregue a página.",
  RATE_LIMITED: "Muitas tentativas em pouco tempo. Aguarde antes de tentar novamente.",
  ...
  ACCESS_REQUEST_ALREADY_PENDING: "Já existe uma solicitação pendente para este Defensor.",
  ...
  CONCURRENT_OPERATION: "Operação já em andamento. Aguarde a conclusão e tente novamente.",
};

const ERROR_CODE_PATTERN = new RegExp(`\\b(${Object.keys(teamErrorMessages).join("|")})\\b`);
