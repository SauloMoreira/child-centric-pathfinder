...
export function CadastrarCriancaSheet({
  ...
  useEffect(() => {
    if (!isEdit) {
      setEffectiveOrgaoId(orgaoId ?? null);
      return;
    }
  ...
  useEffect(() => {
    ...
    const fams: Familiar[] = full.data.vinculos
      .filter((v) => v.origem_id === assistidoId && (v.tipo === "pai" || v.tipo === "mae" || v.tipo === "familia_extensa"))
      .map((v) => ({ assistidoId: v.outro_id, tipo: v.tipo as Familiar["tipo"] }));
    const irms: string[] = full.data.vinculos
      .filter((v) => v.tipo === "irmao")
      .map((v) => v.outro_id);
    setFamiliares(fams);
    setIrmaos(irms);
    setHydrated(true);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(isEdit ? "Cadastro atualizado com sucesso." : "Criança ou adolescente cadastrado com sucesso.");
      ...
      <SheetContent side="right" className="flex w-full flex-col gap-0 p-0 sm:max-w-[560px]">
        <SheetHeader className="border-b border-border p-4">
          <SheetTitle className="flex items-center gap-2 text-base">
            ...
            {isEdit ? "Editar criança ou adolescente" : "Cadastrar criança ou adolescente"}
          </SheetTitle>
          <SheetDescription>
            {isEdit
              ? "Atualize os dados do cadastro e ajuste vínculos familiares e irmãos."
