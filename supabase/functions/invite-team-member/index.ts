...
type Payload = {
  ...
  funcaoInterna: "assessor" | "servidor" | "estagiario" | "residente" | "colaborador" | "outro";
...
Deno.serve(async (req) => {
  ...
  const { data: created, error: rpcErr } = await userClient.rpc("criar_convite_equipe", {
  ...
  if (rpcErr) {
    ...
    return json(
      ...
      code === "UNAUTHENTICATED" || code === "FORBIDDEN" || code === "USER_NOT_ACTIVE" ? 403 : 400,
    );
  }
  ...
  if (inviteErr) {
    ...
    return json({ error: "INVITATION_SEND_FAILED", message: inviteErr.message }, 502);
  }
