-- Ajuste doc (novo AJUSTE 17) — links sugeridos nas cotas: título + url,
-- exibidos ao final da página da cota, antes dos botões.

ALTER TABLE public.content_versions
  ADD COLUMN IF NOT EXISTS links jsonb NOT NULL DEFAULT '[]'::jsonb;

-- ---------------------------------------------------------------------------
-- criar_cota — novo parâmetro opcional p_links.
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.criar_cota(text, jsonb, text, uuid[], text, text);
...
REVOKE ALL ON FUNCTION public.criar_cota(text, jsonb, text, uuid[], text, text, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.criar_cota(text, jsonb, text, uuid[], text, text, jsonb) TO authenticated;

-- ---------------------------------------------------------------------------
-- atualizar_cota — novo parâmetro opcional p_links.
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.atualizar_cota(uuid, bigint, uuid, text, jsonb, text, uuid[], text, text);
...
REVOKE ALL ON FUNCTION public.atualizar_cota(uuid, bigint, uuid, text, jsonb, text, uuid[], text, text, jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.atualizar_cota(uuid, bigint, uuid, text, jsonb, text, uuid[], text, text, jsonb) TO authenticated;

-- ---------------------------------------------------------------------------
-- obter_cota_detalhe — inclui 'links' no retorno.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'links', COALESCE(v_ver.links, '[]'::jsonb),
    'categorias', v_cats,
